...
class AssessmentSpider extends BasicSpider
{
    ...
    {
        $links = $response->filter('a[class="serp-item__title"]')->links();
        foreach ($links as $link) {
            Roach::startSpider(UserSpider::class, context: ['link' => $link->getUri()]);
        }
        yield $this->item([]);
    }
